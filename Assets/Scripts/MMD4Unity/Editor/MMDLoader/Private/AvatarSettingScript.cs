...
public class AvatarSettingScript 
{
    ...
    void SetRequirePose(Transform transform)
    {
        ...
        case "センター":
            if (HasBone("腰") ^ ("センター" == transform.name)) {
                ...
                //後ろを向いているので向き直す
                transform.SetLocalPositionAndRotation(new Vector3(-transform.localPosition.x, transform.localPosition.y, -transform.localPosition.z), Quaternion.Euler(0.0f, 180.0f, 0.0f));
            }
            break;
        default:
    ...
    {
        static HumanBone CreateHint(string key, string value)
        {
            var human_bone = new HumanBone
            {
                humanName = key,
                boneName = value
            };
        ...
        var bone_name = new List<HumanBone>();
        ...
        bone_name.Add(CreateHint("Left Index Distal",           HasBone("左人指先") ? "左人指先" : "左人差指先"));    //左人差し指(先)△
        ...
        bone_name.Add(CreateHint("Right Index Distal",          HasBone("右人指先") ? "右人指先" : "右人差指先")); //右人差し指(先)△
    ...
    SkeletonBone[] CreateSkeletonBone()
    {
        ...
        {
            var skeleton_bone = new SkeletonBone { name = x.name };
    ...
    readonly GameObject root_game_object_ = null;
    readonly GameObject[] bones_ = null;
    Avatar avatar_ = null;
