...
{
    public class PlanarReflectionHelper : MonoBehaviour
    {
        ...
        private void CreateReflectionCamera()
        {
            ...
            // Create camera component and assign
        ...
        private void UpdateCameraProperties(Camera src, Camera dest) {
            ...
            if (dest.gameObject.TryGetComponent(out UniversalAdditionalCameraData camData))
            {
                ...
                camData.renderShadows = useShadowForReflection;
            }
        }

        void Awake()
        {
            if (targetPlane is null)
                Debug.LogError("Reflection plane not assigned!");

            // Create reflection camera if not present
            if (_reflectionCamera is null)
