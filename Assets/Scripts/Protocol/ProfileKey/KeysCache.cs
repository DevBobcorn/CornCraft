...
namespace MinecraftClient.Protocol.Keys
{
    ...
    public static class KeysCache
    {
        ...
        public static void Store(string login, PlayerKeyPair playerKeyPair)
        {
            ...
            if (CornGlobal.ProfileKeyCaching == CacheType.Disk && updatetimer.Enabled == true)
            {
                pendingadds.Add(new KeyValuePair<string, PlayerKeyPair>(login, playerKeyPair));
            }
            else if (CornGlobal.ProfileKeyCaching == CacheType.Disk)
            {
                SaveToDisk();
            }
        }
        ...
        private static bool LoadFromDisk()
        {
            ...
            if (File.Exists(KeysCacheFilePlaintext))
            {
                if (CornGlobal.DebugMode)
                    Translations.Log("cache.loading_keys", KeysCacheFilePlaintext);
                        ...
                        if (!line.TrimStart().StartsWith("#"))
                        {
                            ...
                            if (separatorIdx >= 1 && line.Length > separatorIdx + 1)
                            {
                                ...
                                try
                                {
                                    ...
                                    keys[login] = playerKeyPair;
                                    if (CornGlobal.DebugMode)
                                        Translations.Log("cache.loaded_keys", playerKeyPair.ExpiresAt.ToString());
                                }
                                catch (InvalidDataException e)
                                {
                                    if (CornGlobal.DebugMode)
                                        Translations.Log("cache.ignore_string_keys", value, e.Message);
                                }
                            ...
                            {
                                Translations.Log("cache.ignore_line_keys", line);
                            }
                        }
                    }
                }
                ...
                {
                    Translations.Log("cache.read_fail_plain_keys", e.Message);
                }
            }
        ...
        {
            if (CornGlobal.DebugMode)
                Translations.Log("cache.saving_keys");

            List<string> KeysCacheLines = new List<string>();
            KeysCacheLines.Add("# Generated by CornClient " + CornGlobal.Version + " - Edit at own risk!");
            ...
            {
                Translations.Log("cache.save_fail_keys", e.Message);
            }
        }
    }
}