            ...
            if (hasArg(command))
            {
               ...
               if (args.Length > 1)
               {
                    ...
                    if (args.Length == 2 && args[1].Contains("to"))
                    {
                        ...
                        if (CornCraft.SetVar(args[0], rand.Next(num1, num2)))
                        {
                            return string.Format("Set %{0}% to {1}.", args[0], CornCraft.GetVar(args[0])); //Success
                        }
                    ...
                    else
                    {
                        ...
                        if (values.Count > 0 && CornCraft.SetVar(args[0], values[rand.Next(0, values.Count)]))
                        {
                            return string.Format("Set %{0}% to {1}.", args[0], CornCraft.GetVar(args[0])); //Success
                        }
