...
namespace CraftSharp.Protocol.Handlers.PacketPalettes
{
    public class PacketPalette1216 : PacketTypePalette
    {
        private readonly Dictionary<int, PacketTypesIn> typeIn = new()
        {
            ...
            { 0x83, PacketTypesIn.Waypoint },                   // Added in 1.21.6
            { 0x84, PacketTypesIn.ClearDialog },                // Added in 1.21.6
            { 0x85, PacketTypesIn.ShowDialog }                  // Added in 1.21.6
        };
        ...
        private readonly Dictionary<int, PacketTypesOut> typeOut = new()
        {
            ...
            { 0x04, PacketTypesOut.ChangeGameMode },              // Added in 1.21.6 (Previously achieved using a command)
            { 0x05, PacketTypesOut.MessageAcknowledgment },       // Added in 1.19.1
            { 0x06, PacketTypesOut.ChatCommand },                 // Added in 1.19
            { 0x07, PacketTypesOut.SignedChatCommand },           // Added in 1.20.6
            { 0x08, PacketTypesOut.ChatMessage },                 // Changed in 1.19 (Completely changed) (Wiki name: Chat)
            { 0x09, PacketTypesOut.PlayerSession },               // Added in 1.19.3
            { 0x0A, PacketTypesOut.ChunkBatchReceived },          // Added in 1.20.2 
            { 0x0B, PacketTypesOut.ClientStatus },                // (Wiki name: Client Command)
            { 0x0C, PacketTypesOut.ClientTickEnd },               // Added in 1.21.2 (Wiki name: Client Tick End)
            { 0x0D, PacketTypesOut.ClientSettings },              // (Wiki name: Client Information)
            { 0x0E, PacketTypesOut.TabComplete },                 // (Wiki name: Command Suggestions Request)
            { 0x0F, PacketTypesOut.AcknowledgeConfiguration },    // Added in 1.20.2 
            { 0x10, PacketTypesOut.ClickInventoryButton },        // (Wiki name: Click Container Button)
            { 0x11, PacketTypesOut.ClickInventory },              // (Wiki name: Click Container)
            { 0x12, PacketTypesOut.CloseInventory },              // (Wiki name: Close Container (serverbound))
            { 0x13, PacketTypesOut.ChangeInventorySlotState },    // Added in 1.20.3
            { 0x14, PacketTypesOut.CookieResponse },              // Added in 1.20.6
            { 0x15, PacketTypesOut.PluginMessage },               // (Wiki name: Serverbound Plugin Message)
            { 0x16, PacketTypesOut.DebugSampleSubscription },     // Added in 1.20.6
            { 0x17, PacketTypesOut.EditBook },                    //
            { 0x18, PacketTypesOut.EntityNBTRequest },            // (Wiki name: Query Entity Tag)
            { 0x19, PacketTypesOut.InteractEntity },              // (Wiki name: Interact)
            { 0x1A, PacketTypesOut.GenerateStructure },           // (Wiki name: Jigsaw Generate)
            { 0x1B, PacketTypesOut.KeepAlive },                   // (Wiki name: Serverbound Keep Alive (play))
            { 0x1C, PacketTypesOut.LockDifficulty },              //
            { 0x1D, PacketTypesOut.PlayerPosition },              // (Wiki name: Set Player Position)
            { 0x1E, PacketTypesOut.PlayerPositionAndRotation },   // (Wiki name: Set Player Position and Rotation)
            { 0x1F, PacketTypesOut.PlayerRotation },              // (Wiki name: Set Player Rotation)
            { 0x20, PacketTypesOut.PlayerMovement },              // (Wiki name: Set Player Movement Flags)
            { 0x21, PacketTypesOut.VehicleMove },                 // (Wiki name: Move Vehicle (serverbound))
            { 0x22, PacketTypesOut.SteerBoat },                   // (Wiki name: Paddle Boat)                   //
            { 0x23, PacketTypesOut.PickItemFromBlock },           // Added in 1.21.4
            { 0x24, PacketTypesOut.PickItemFromEntity },          // Added in 1.21.4
            { 0x25, PacketTypesOut.PingRequest },                 // Added in 1.20.2 
            { 0x26, PacketTypesOut.CraftRecipeRequest },          // (Wiki name: Place recipe)
            { 0x27, PacketTypesOut.PlayerAbilities },             //
            { 0x28, PacketTypesOut.PlayerDigging },               // Changed in 1.19 (Added a "Sequence" field) (Wiki name: Player Action) 
            { 0x29, PacketTypesOut.EntityAction },                // (Wiki name: Player Command)
            { 0x2A, PacketTypesOut.SteerVehicle },                // (Wiki name: Player Input)
            { 0x2B, PacketTypesOut.PlayerLoaded },                // Added in 1.21.4
            { 0x2C, PacketTypesOut.Pong },                        // (Wiki name: Pong (play))
            { 0x2D, PacketTypesOut.SetDisplayedRecipe },          // (Wiki name: Recipe Book Change Settings)
            { 0x2E, PacketTypesOut.SetRecipeBookState },          // (Wiki name: Recipe Book Seen Recipe)
            { 0x2F, PacketTypesOut.NameItem },                    // (Wiki name: Rename Item)
            { 0x30, PacketTypesOut.ResourcePackStatus },          // (Wiki name: Resource Pack (serverbound))
            { 0x31, PacketTypesOut.AdvancementTab },              // (Wiki name: Seen Advancements)
            { 0x32, PacketTypesOut.SelectTrade },                 //
            { 0x33, PacketTypesOut.SetBeaconEffect },             // Changed in 1.19 (No need to be implemented yet)
            { 0x34, PacketTypesOut.HeldItemChange },              // (Wiki name: Set Carried Item (serverbound))
            { 0x35, PacketTypesOut.UpdateCommandBlock },          // (Wiki name: Program Command Block)
            { 0x36, PacketTypesOut.UpdateCommandBlockMinecart },  // (Wiki name: Program Command Block Minecart)
            { 0x37, PacketTypesOut.CreativeInventoryAction },     // (Wiki name: Set Creative Mode Slot)
            { 0x38, PacketTypesOut.UpdateJigsawBlock },           // (Wiki name: Program Jigsaw Block)
            { 0x39, PacketTypesOut.UpdateStructureBlock },        // (Wiki name: Program Structure Block)
            { 0x3A, PacketTypesOut.UpdateTestBlock },             // Added in 1.21.5 (Wiki name: Set Test Block)
            { 0x3B, PacketTypesOut.UpdateSign },                  // (Wiki name: Update Sign)
            { 0x3C, PacketTypesOut.Animation },                   // (Wiki name: Swing Arm)
            { 0x3D, PacketTypesOut.Spectate },                    // (Wiki name: Teleport To Entity)
            { 0x3E, PacketTypesOut.TestInstanceBlockAction },     // Added in 1.21.5
            { 0x3F, PacketTypesOut.PlayerBlockPlacement },        // Changed in 1.19 (Added a "Sequence" field) (Wiki name: Use Item On) 
            { 0x40, PacketTypesOut.UseItem },                     // Changed in 1.19 (Added a "Sequence" field) (Wiki name: Use Item) 
            { 0x41, PacketTypesOut.CustomClickAction }            // Added in 1.21.6
        };
        ...
        private readonly Dictionary<int, ConfigurationPacketTypesIn> configurationTypesIn = new()
        {
            ...
            { 0x11, ConfigurationPacketTypesIn.ClearDialog },         // Added in 1.21.6
            { 0x12, ConfigurationPacketTypesIn.ShowDialog }           // Added in 1.21.6
        };
        ...
        private readonly Dictionary<int, ConfigurationPacketTypesOut> configurationTypesOut = new()
        {
            ...
            { 0x08, ConfigurationPacketTypesOut.CustomClickAction }     // Added in 1.21.6
        };
